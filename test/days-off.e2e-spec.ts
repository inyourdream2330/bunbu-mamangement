...
describe('Days Off Controller E2E Test', () => {
  ...
  describe('Find one day off', () => {
    ...
    it('Find one day off success', async () => {
      return await request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body.message).toBe(
            `Day off id = ${createDaysOff.data.id} get data success`,
          );
        });
    });

    it('Find one day off fail, missing auth token', async () => {
      return await request(app.getHttpServer())
        .get(`/days-off/${createDaysOff.data.id}`)
        .expect(HttpStatus.UNAUTHORIZED)
        .expect((res) => {
    ...
    it('Find one day off fail, day off not exist', async () => {
      const fakeId = -1;
      return await request(app.getHttpServer())
        .get(`/days-off/${fakeId}`)
        .set('Authorization', 'Bearer ' + loginUserAccessToken)
